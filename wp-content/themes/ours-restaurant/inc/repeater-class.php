...
if (class_exists('WP_Customize_control')) {
    ...
    class Ours_Restaurant_Repeater_Controler extends WP_Customize_Control
    {
        /**
         * The control type.
         *
        ...
        public $ours_restaurant_box_label = '';

        public $ours_restaurant_box_add_control = '';
        ...
        {
            $this->fields = $fields;
            $this->ours_restaurant_box_label = $args['ours_restaurant_box_label'];
            $this->ours_restaurant_box_add_control = $args['ours_restaurant_box_add_control'];
        ...
        <?php } ?>
            ...
            <ul class="ours_restaurant-repeater-field-control-wrap">
                <?php
                $this->ours_restaurant_get_fields();
                ?>
            </ul>

            <input type="hidden" <?php $this->link(); ?> class="ours_restaurant-repeater-collector"
                   value="<?php echo esc_attr($this->value()); ?>"/>
            <button type="button"
                    class="button ours_restaurant-add-control-field"><?php echo esc_html($this->ours_restaurant_box_add_control); ?></button>
            <?php
        }

        private function ours_restaurant_get_fields()
        {
            $fields = $this->fields;
            $values = json_decode($this->value());
            ...
            if (is_array($values)) {
                foreach ($values as $value) {
                    ?>
                    <li class="ours_restaurant-repeater-field-control">
                        <h3 class="ours_restaurant-repeater-field-title"><?php echo esc_html($this->ours_restaurant_box_label); ?></h3>

                        <div class="ours_restaurant-repeater-fields">
                            <?php
                            foreach ($fields as $key => $field) {
                                $class = isset($field['class']) ? $field['class'] : '';
                                ?>
                                <div
                                        class="ours_restaurant-fields ours_restaurant-type-<?php echo esc_attr($field['type']) . ' ' . $class ?>">
                                    ...
                                    switch ($field['type']) {
                                        ...
                                        case 'upload':
                                            ...
                                            echo '<div class="ours_restaurant-fields-wrap">';
                                            echo '<div class="attachment-media-view">';
                                            echo '<div class="placeholder' . $image_class . '">';
                                            esc_html__('No image selected', 'ours-restaurant');
                                            echo '</div>';
                                            echo '<div class="thumbnail thumbnail-image">';
                                            echo $image;
                                            echo '</div>';
                                            echo '<div class="actions clearfix">';
                                            echo '<button type="button" class="button ours_restaurant-delete-button align-left">' . esc_html__('Remove', 'ours-restaurant') . '</button>';
                                            echo '<button type="button" class="button ours_restaurant-upload-button alignright">' . esc_html__('Select Image', 'ours-restaurant') . '</button>';
                                        ...
                                        case 'category':
                                            ...
                                            echo '<option value="0">' . esc_html__('Select Category', 'ours-restaurant') . '</option>';
                                            echo '<option value="-1">' . esc_html__('Latest Posts', 'ours-restaurant') . '</option>';
                                        ...
                                        case 'colorpicker':
                                            echo '<input data-default="' . esc_attr($default) . '" class="ours_restaurant-color-picker" data-alpha="true" data-name="' . esc_attr($key) . '" type="text" value="' . esc_attr($new_value) . '"/>';
                                            break;
                                        ...
                                        case 'range':
                                            ...
                                            echo '<div class="ours_restaurant-range-slider" >';
                                        ...
                                        case 'icon':
                                            echo '<div class="ours_restaurant-selected-icon">';
                                            echo '<i class="' . esc_attr($new_value) . '"></i>';
                                            echo '<span><i class="fa fa-angle-down"></i></span>';
                                            echo '</div>';
                                            echo '<ul class="ours_restaurant-icon-list clearfix">';
                                            $ours_restaurant_font_awesome_icon_array = ours_restaurant_font_awesome_icon_array();
                                            foreach ($ours_restaurant_font_awesome_icon_array as $ours_restaurant_font_awesome_icon) {
                                                $icon_class = $new_value == $ours_restaurant_font_awesome_icon ? 'icon-active' : '';
                                                echo '<li class=' . $icon_class . '><i class="' . $ours_restaurant_font_awesome_icon . '"></i></li>';
                                            }
                                            echo '</ul>';
                                            echo '<input data-default="' . esc_attr($default) . '" type="hidden" value="' . esc_attr($new_value) . '" data-name="' . esc_attr($key) . '"/>';
                                            break;
                                        ...
                                        case 'multicategory':
                                            ...
                                            echo '<ul class="ours_restaurant-multi-category-list">';
                                            echo '<li><label><input type="checkbox" value="-1" ' . checked('-1', $new_value, false) . '/>' . esc_html_e('Latest Posts', 'ours-restaurant') . '</label></li>';
                                            foreach ($this->cats as $cat) {
                            ...
                            <div class="clearfix ours_restaurant-repeater-footer">
                                <div class="alignright">
                                    <a class="ours_restaurant-repeater-field-remove"
                                       href="#remove"><?php esc_html_e('Delete', 'ours-restaurant') ?></a> |
                                    <a class="ours_restaurant-repeater-field-close"
                                       href="#close"><?php esc_html_e('Close', 'ours-restaurant') ?></a>
                                </div>
                            </div>
                        </div>
                    </li>
...
if (!function_exists('ours_restaurant_font_awesome_icon_array')) {
    function ours_restaurant_font_awesome_icon_array()
    {
